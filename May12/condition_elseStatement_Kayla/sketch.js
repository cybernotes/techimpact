...
var circR = 255; // creates variable for R value of color
var circG = 0; // creates variable for G value of color
var circB = 0; //creates variable for B value of color
...
function setup() {
  createCanvas(400, 400); //creates canvas size
}
...
function draw() {
  ...
  ellipse(circX + 50, circY - 75, circSize, circSize); //creates a second circle by adding and subtracting to the x y variables to seperate circle
  ...
  circX = circX + speed; //changes the circle x coordinate by adding speed variable
  recY = recY - speed; // changes the rectangle y cordinate by adding speed variable

  if (circX < width) { // if the circle x is less than width

    circSize += 0.5; // add .5 to the circle size

  } else { //otherwise
    if (circX > width) { // if the circle x is greater than the width 
